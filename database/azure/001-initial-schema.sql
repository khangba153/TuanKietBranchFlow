-- Chạy toàn bộ file trong SSMS với Query > SQLCMD Mode được bật.
-- Dừng ở lỗi đầu tiên; không chạy riêng phần được bôi đen.
:on error exit
...
-- Chỉ khởi tạo Azure SQL demo đã tồn tại và chưa có bảng nghiệp vụ.
IF CONVERT(INT, SERVERPROPERTY('EngineEdition')) <> 5
    THROW 51000, N'Chỉ chạy script này trên Azure SQL Database.', 1;
IF DB_NAME() <> N'BranchFlowDB-Demo'
    THROW 51001, N'Sai database đích. Hãy chọn BranchFlowDB-Demo.', 1;
IF EXISTS (SELECT 1 FROM sys.tables WHERE is_ms_shipped = 0)
    THROW 51002, N'Database đã có bảng; không chạy lại script khởi tạo.', 1;
GO

-- Giữ schema và seed cùng một transaction xuyên các batch GO.
-- Khi có lỗi: không COMMIT; đóng kết nối để rollback trước khi thử lại.
BEGIN TRANSACTION;
GO
...
PRINT N'Azure schema và seed cấu hình đã tạo thành công.';
